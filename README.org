...
**** Move the player code into service
      ...
      First, delete the =MediaService= constructor that was created by the Android studio. Next, we'll gradually migrate all =MediaPlayer= code (and the the =random= instance) into =MediaService= class.
...
***** Add class =LocalBinder=
       Inside our =MediaService=, define the =LocalBinder= as a static inner class. Use the following code.
       #+BEGIN_SRC java
